...
public class MemoryPickContactsActivity extends BaseActivity{
    /** if this is a new group */
    protected boolean isCreatingNewGroup;
    private MemoryPickContactsActivity.PickContactAdapter contactAdapter;
...
//                Log.e("zxd","获取到的成员id"+memoryFriend.getAddTime());
        ...
        existMembers.add(EMClient.getInstance().getCurrentUser());
        ...
        contactAdapter = new MemoryPickContactsActivity.PickContactAdapter(this, R.layout.em_row_contact_with_checkbox, alluserList);
    ...
    public void save(View v) {
        ...
        for (int i=0;i<var.size();i++){
            ...
            addMemoryBookFriencPost(memoryBookId,var.get(i));
        }
        if(flag==false){
            Toast.makeText(MemoryPickContactsActivity.this,"保存成功",Toast.LENGTH_SHORT).show();
        }else {
            Toast.makeText(MemoryPickContactsActivity.this,"保存失败",Toast.LENGTH_SHORT).show();
        }
...
//                            Toast.makeText(MemoryPickContactsActivity.this,"获取成员失败",Toast.LENGTH_SHORT).show();
    ...
    public void addMemoryBookFriencPost(final String memoryBookId,final String friendId) {
        final List<OkHttpUtils.Param> list = new ArrayList<OkHttpUtils.Param>();
        //可以传多个参数，这里只写传一个参数，需要传多个参数时list.add();
        OkHttpUtils.Param bookIdParam = new OkHttpUtils.Param("memoryBookId", memoryBookId);
        OkHttpUtils.Param friendIdParam = new OkHttpUtils.Param("friendId", friendId);
        list.add(bookIdParam);
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                //post方式连接  url，post方式请求必须传参
                //参数方式：OkHttpUtils.post(url,OkHttpUtils.ResultCallback(),list)
                OkHttpUtils.post(APPConfig.addMemoryFriend, new OkHttpUtils.ResultCallback() {
                    @Override
                    public void onSuccess(Object response) {
                        Log.d("testRun", "response------" + response.toString());
                        try {// 不要在这个try catch里对ResultDto进行调用，因为这里解析json数据可能会因为后台出错等各种问题导致解析结果异常
                        ...
                        if(memoryFriendListResult.getMsg().equals("addMemoryFriend_success")){
                            //sUser.setmName(resultDto.getData().getName());
//                            initListData(memoryBookListResult.getData());
//                            mAdapter.notifyDataSetChanged();
                            Log.e("zxd","添加成功");
